...
import static tndown.tndir.simplevd.vdwload.Utils.isBlank;
...
public final class YoutubeOtfDashManifestCreator {
    ...
    public static String fromOtfStreamingUrl(
        ...
        // from video servers.
        final Response response = YoutubeDashManifestCreatorsUtils.getInitializationResponse(realOtfBaseStreamingUrl,
                itagItem, DeliveryType.OTF);
        realOtfBaseStreamingUrl = response.latestUrl().replace(YoutubeDashManifestCreatorsUtils.SQ_0, EMPTY_STRING)
                .replace(YoutubeDashManifestCreatorsUtils.RN_0, EMPTY_STRING).replace(YoutubeDashManifestCreatorsUtils.ALR_YES, EMPTY_STRING);
    ...
    private static void generateSegmentElementsForOtfStreams(
        ...
        try {
            final Element segmentTimelineElement = (Element) doc.getElementsByTagName(
                    YoutubeDashManifestCreatorsUtils.SEGMENT_TIMELINE).item(0);

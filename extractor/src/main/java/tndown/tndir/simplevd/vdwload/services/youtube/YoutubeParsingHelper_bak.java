...
package tndown.tndir.simplevd.vdwload.services.youtube;
...
import tndown.tndir.simplevd.vdwload.exceptions.AccountTerminatedException;
...
import tndown.tndir.simplevd.vdwload.localization.Localization;
...
import tndown.tndir.simplevd.vdwload.utils.RandomStringFromAlphabetGenerator;
...
import static tndown.tndir.simplevd.vdwload.Utils.getStringResultFromRegexArray;
import static tndown.tndir.simplevd.vdwload.Utils.isNullOrEmpty;

public final class YoutubeParsingHelper_bak {
    ...
    private YoutubeParsingHelper_bak() {
    }
    ...
    public static boolean areHardcodedClientVersionAndKeyValid()
        ...
        final Response response = NewPipe.getDownloader().post(YOUTUBEI_V1_URL + "guide?key="
    ...
    private static void extractClientVersionAndKeyFromSwJs()
        ...
        final String response = NewPipe.getDownloader().get(url, headers).responseBody();
        try {
    ...
    private static void extractClientVersionAndKeyFromHtmlSearchResultsPage()
        ...
        final String html = NewPipe.getDownloader().get(url, headers).responseBody();
        ...
        if (!Utils.isNullOrEmpty(clientVersion) && !Utils.isNullOrEmpty(shortClientVersion)) {
    ...
    public static boolean isHardcodedYoutubeMusicKeyValid() throws IOException,
        ...
        final byte[] json = JsonWriter.string()
                ...
                .value("experimentsToken", Utils.EMPTY_STRING)
        ...
        final Response response = NewPipe.getDownloader().post(url, headers, json);
            ...
            if (internUrl.startsWith("/redirect?")) {
                ...
                for (final String param : params) {
                    if (param.split("=")[0].equals("q")) {
                        try {
                            return URLDecoder.decode(param.split("=")[1], Utils.UTF_8);
    ...
    public static JsonObject getJsonPostResponse(final String endpoint,
        ...
        final Response response = NewPipe.getDownloader().post(YOUTUBEI_V1_URL + endpoint + "?key="
    ...
    private static JsonObject getMobilePostResponse(
        ...
        final Response response = NewPipe.getDownloader().post(Utils.isNullOrEmpty(endPartOfUrlRequest)
    ...
    public static void defaultAlertsCheck(@Nonnull final JsonObject initialData)
        ...
        if (!Utils.isNullOrEmpty(alerts)) {
            ...
            final String alertType = alertRenderer.getString("type", Utils.EMPTY_STRING);
    ...
    private static MetaInfo getInfoPanelContent(@Nonnull final JsonObject infoPanelContentRenderer)
        ...
        for (final Object paragraph : infoPanelContentRenderer.getArray("paragraphs")) {
            ...
            sb.append(YoutubeParsingHelper_bak.getTextFromObject((JsonObject) paragraph));
        }
        metaInfo.setContent(new Description(sb.toString(), Description.HTML));
        if (infoPanelContentRenderer.has("sourceEndpoint")) {
            final String metaInfoLinkUrl = YoutubeParsingHelper_bak.getUrlFromNavigationEndpoint(
            ...
            final String metaInfoLinkText = YoutubeParsingHelper_bak.getTextFromObject(
    ...
    private static MetaInfo getClarificationRendererContent(
        ...
        final String title = YoutubeParsingHelper_bak.getTextFromObject(clarificationRenderer
                .getObject("contentTitle"));
        final String text = YoutubeParsingHelper_bak.getTextFromObject(clarificationRenderer
                .getObject("text"));
        ...
        if (clarificationRenderer.has("actionButton")) {
            ...
            try {
                final String url = YoutubeParsingHelper_bak.getUrlFromNavigationEndpoint(actionButton
                        .getObject("command"));

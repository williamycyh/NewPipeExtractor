...
import com.github.video.downloader.services.soundcloud.extractors.SoundcloudStreamInfoItemExtractor;
...
import com.github.video.downloader.services.soundcloud.extractors.SoundcloudChannelInfoItemExtractor;
...
public final class SoundcloudParsingHelper {
    ...
    public static JsonObject resolveFor(@Nonnull final Downloader downloader, final String url)
        ...
        final String apiUrl = SOUNDCLOUD_API_V2_URL + "resolve"
                + "?url=" + URLEncoder.encode(url, Utils.UTF_8)
                + "&client_id=" + clientId();

        try {
            final String response = downloader.get(apiUrl, ServiceList.SoundCloud.getLocalization())
                    .responseBody();
    ...
    public static String resolveUrlWithEmbedPlayer(final String apiUrl) throws IOException,
        ...
        final String response = NewPipe.getDownloader().get("https://w.soundcloud.com/player/?url="
                + URLEncoder.encode(apiUrl, Utils.UTF_8), ServiceList.SoundCloud.getLocalization()).responseBody();
    ...
    public static String resolveIdWithWidgetApi(final String urlString) throws IOException,
        ...
        try {
            final String widgetUrl = "https://api-widget.soundcloud.com/resolve?url="
                    + URLEncoder.encode(url.toString(), Utils.UTF_8)
    ...
    public static String getUsersFromApi(final ChannelInfoItemsCollector collector,
                                         final String apiUrl) throws IOException,
            ReCaptchaException, ParsingException {
        final String response = NewPipe.getDownloader().get(apiUrl, ServiceList.SoundCloud.getLocalization())
                .responseBody();
    ...
    public static String getStreamsFromApi(final StreamInfoItemsCollector collector,
            ...
            ReCaptchaException, ParsingException {
        final Response response = NewPipe.getDownloader().get(apiUrl, ServiceList.SoundCloud
                .getLocalization());
    ...
    public static String getUploaderUrl(final JsonObject object) {
        final String url = object.getObject("user").getString("permalink_url", Utils.EMPTY_STRING);
        return Utils.replaceHttpWithHttps(url);
    }

    @Nonnull
    public static String getAvatarUrl(final JsonObject object) {
        final String url = object.getObject("user").getString("avatar_url", Utils.EMPTY_STRING);
        return Utils.replaceHttpWithHttps(url);
    }

    public static String getUploaderName(final JsonObject object) {
        return object.getObject("user").getString("username", Utils.EMPTY_STRING);
    }
}
